...
const ShortenItem = ({ originalUrl, shortUrl, clickCount, createdDate }) => {
  ...
  const subDomain = import.meta.env.VITE_FRONT_END_URL.replace(
    /^https?:\/\//,
    ""
  );
  ...
  const fetchMyShortUrl = async () => {
    ...
    try {
      const { data } = await api.get(
        `/api/urls/analytics/${selectedUrl}?startDate=2025-03-01T00:00:00&endDate=2025-03-22T23:59:59`,
        {
          headers: {
      ...
      <div className="flex sm:flex-row flex-col sm:justify-between w-full sm:gap-0 gap-5 py-5">
        <div className="flex-1 sm:space-y-1 max-w-full overflow-x-auto overflow-y-hidden">
          <div className="text-slate-900 pb-1 sm:pb-0 flex items-center gap-2">
            {/* <a
              href={`${import.meta.env.VITE_FRONT_END_URL}/${shortUrl}`}
              target="_blank"
            ...
            <Link
              target="_"
              className="text-[17px] font-montserrat font-[600] text-linkColor"
              to={import.meta.env.VITE_FRONT_END_URL + `/s/${shortUrl}`}
            >
        ...
        <div className="flex flex-1 sm:justify-end items-center gap-4">
          ...
          >
            <div className="flex cursor-pointer gap-1 items-center btn-color py-2 font-semibold shadow-md shadow-slate-500 px-6 rounded-md text-white">
